Function Get-psEventLog {
    ...
    $logs = Get-WinEvent -LogName Security -FilterXPath "*[System[(EventID=4688)]]" -MaxEvents 25
    ...
    foreach ($entries in $logs) {
        ...
        $out | Add-Member -MemberType NoteProperty -Name Timestamp -Value $entries.TimeCreated
        $out | Add-Member -MemberType NoteProperty -Name PID -Value $ProcID
        $out | Add-Member -MemberType NoteProperty -Name Process -Value $ProcName
        $out | Add-Member -MemberType NoteProperty -Name Domain -Value $Domain
        $out | Add-Member -MemberType NoteProperty -Name User -Value $Account            
        $out | Add-Member -MemberType NoteProperty -Name PPID -Value $PPID
        $out | Add-Member -MemberType NoteProperty -Name ParentProcess -Value $PProcName
        $out

    } 
}    

# Pipe the output to any format you like or view
Get-psEventLog | Out-GridView
